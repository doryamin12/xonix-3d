...
- **`AudioContext` נוצר/משוחרר רק דרך `SFX.unlock()`, שנקרא מ-`Game.beginNewGame()`.** דפדפנים חוסמים אודיו ללא user gesture אמיתי - קריאה פרוגרמטית (למשל דרך אוטומציה/סקריפט) לא תפעיל בפועל את הסאונד (ה-context יישאר `suspended`), רק קליק/הקשה אמיתיים של משתמש יעבדו. זו לא בעיה בקוד - זו מדיניות הדפדפן, ואי אפשר לעקוף אותה.
